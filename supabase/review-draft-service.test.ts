...
const input: ReviewDraftInput = { userId: 'u1', asin: 'B000000001', reviewFingerprint: 'review-fingerprint-1', stars: 2, reviewText: 'The zipper broke.', brandProfileId: null, extraInstruction: null, requestId: 'req-1' };
const calls: string[] = [];
...
await assert.rejects(() => generateReviewDraft({ ...input, reviewFingerprint: '   ' }, repo, async () => ({ draft: 'x', analysis: 'x', modelVersion: 'test' })), /invalid-review-fingerprint/);
...
await assert.rejects(() => generateReviewDraft(input, { ...repo, recordDraftAndUsage: async () => { throw new Error('duplicate-request-id'); } }, async () => ({ draft: 'x', analysis: 'x', modelVersion: 'test' })), /duplicate-request-id/);
await assert.rejects(() => generateReviewDraft(input, { ...repo, recordDraftAndUsage: async () => { throw new Error('review-generation-quota-reached'); } }, async () => ({ draft: 'x', analysis: 'x', modelVersion: 'test' })), /quota/);
